 ...
 function send_notificationToken ($tokens, $message,$key)
    ...
    public function  api_followingstopAction(Request $request,$user,$token)
    {
        ...
        foreach ($user->getUsers() as $key => $e) {
            if (sizeof($e->getRingtones())>0) {
               $b["id"] = $e->getId();
               $b["name"] = $e->getName();
               $b["image"] = $e->getImage();
               $b["trusted"] = $e->getTrusted();
               $last_ringtone = $em->getRepository('AppBundle:Ringtone')->findOneBy(array("enabled"=>true,"user"=>$e),array("created"=>"desc"));
               if ($last_ringtone!=null) {
                    $b["ringtone"] = $last_ringtone;
                    $users[]= $b;
               }
